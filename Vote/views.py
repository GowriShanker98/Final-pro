...
def home(request):
    if request.user.is_authenticated:
        ...
        return render(request,"Vote/home.html",context={"user":user})
...
def vote(request):
    ...
    if request.method=="POST":
        ...
        c=VoterDetails.objects.filter(user=u).update(is_voted=True)
        return render(request,"Vote/home.html",context={"username":user})
    else:
        v=VoterDetails.objects.get(user=u)
        candidates=Candidate.objects.filter(constituency=v.constituency)
    return render(request,"Vote/vote.html",context={"candidates":candidates,"vote_status":v.is_voted})
...
def register(request):
    ...
    userform=UserForm()
    constform=VoterForm()
    return render(request,'register.html',context={'registered':registered,'userform':userform,'constform':constform})
